...
class CPawn : public ChessPiece
{
public:
	CPawn(char cColor) : ChessPiece(cColor) {}
	~CPawn() {}
private:
...
class CKnight : public ChessPiece
{
public:
	CKnight(char cColor) : ChessPiece(cColor) {}
	~CKnight() {}
private:
	virtual char GetPiece() {
		return 'N';
...
class CBishop : public ChessPiece
{
public:
	CBishop(char cColor) : ChessPiece(cColor) {}
	~CBishop() {}
private:
	virtual char GetPiece() {
		return 'B';
...
class CRook : public ChessPiece
{
public:
	CRook(char cColor) : ChessPiece(cColor) {}
	~CRook() {}
private:
	virtual char GetPiece() {
		return 'R';
...
class CQueen : public ChessPiece
{
public:
	CQueen(char cColor) : ChessPiece(cColor) {}
	~CQueen() {}
private:
	virtual char GetPiece() {
		return 'Q';
...
class CKing : public ChessPiece
{
public:
	CKing(char cColor) : ChessPiece(cColor) {}
	~CKing() {}
private:
	virtual char GetPiece() {
		return 'K';
...
public:
	...
	void GetNextMove(ChessPiece* qpaaBoard[8][8]) {
		...
		do {
			...
			cout << currentPlayerTurn << "'s Move (input 2 digit number for its location): ";
			int iStartMove;
			cin >> iStartMove;
			int iStartRow = (iStartMove / 10) - 1;
			int iStartCol = (iStartMove % 10) - 1;

			cout << "To(2 digit number): ";
			int iEndMove;
			cin >> iEndMove;
			...
			if ((iStartRow >= 0 && iStartRow <= 7) &&
				...
				if ((qpCurrPiece != 0) && (qpCurrPiece->GetColor() == currentPlayerTurn)) {
					...
					if (qpCurrPiece->IsLegalMove(iStartRow, iStartCol, iEndRow, iEndCol, qpaaBoard)) {
						...
						if (!mqGameBoard.IsInCheck(currentPlayerTurn)) {
							delete qpTemp;
							bValidMove = true;
	...
	bool IsGameOver() {
		bool bCanMove(false);
		bCanMove = mqGameBoard.CanMove(currentPlayerTurn);
		if (!bCanMove) {
			if (mqGameBoard.IsInCheck(currentPlayerTurn)) {
				AlternateTurn();
				std::cout << "Checkmate, " << currentPlayerTurn << " Wins!" << std::endl;
			} else {
